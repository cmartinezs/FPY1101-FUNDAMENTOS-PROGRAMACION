# Leccion 03 - Tipos de datos y conversion

Esta leccion explica los tipos de datos basicos en Python y las conversiones entre ellos.

## Idea central

Python infiere el tipo de dato automaticamente, pero es fundamental conocer los tipos disponibles para manipular datos correctamente y evitar errores.

## Estructura de la leccion

- [`teoria.md`](teoria.md): conceptos y ejemplos
- [`aplicacion.md`](aplicacion.md): ejercicios resueltos
- [`practica.md`](practica.md): ejercicios propuestos
- [`soluciones.md`](soluciones.md): ejemplos de soluciones para comparar

## Navegacion

- [Teoria](teoria.md)
- [Aplicacion](aplicacion.md)
- [Practica](practica.md)
- [Soluciones](soluciones.md)
- [Volver a lessons](../README.md)
